...
- 优先使用默认值，不出错不问用户
- 明确区分 Websoft9 首次准备 与 WordPress 站点初始化
...
│  │                                        │  │
│  │                                        │  │
│  │   🎯  Websoft9 for WordPress 即将就绪   │  │
│  │                                        │  │
│  │   只需完成 Websoft9 的首次准备，即可     │  │
│  │   启动 WordPress。                      │  │
│  │                                        │  │
│  │   1. 创建 Websoft9 管理员账号           │  │
│  │   2. 启动 WordPress                    │  │
...
**文案要点:**
- 标题强调这是 Websoft9 for WordPress 的首次准备
- 步骤预览只列 3 条，让用户有预期
- 按钮写"开始"，不写"开始安装"或"初始化"
- 不出现端口、镜像、容器等词汇
- 明确提示 WordPress 站点初始化仍会在 WordPress 内继续完成
...
**文案要点:**
- 标题必须表明这是 Websoft9 管理员账号
- 辅助说明要更像平台定位，说明 Websoft9 是应用运行、访问和维护的统一入口
...
│  │      🚀  一切就绪，启动 WordPress       │  │
│  │                                        │  │
│  │   Websoft9 将自动完成最后的准备工作并   │  │
│  │   启动 WordPress。                      │  │
│  │                                        │  │
│  │   WordPress 启动后，您还会在应用中      │  │
│  │   继续完成站点设置。                    │  │
│  │                                        │  │
│  │         ┌──────────────────┐           │  │
│  │         │ 开始使用 WordPress │           │  │
...
- 按钮文案:"开始使用 WordPress"
- 正常情况无参数输入表单
- 补一句说明：WordPress 启动后，你还会在 WordPress 中完成站点设置
...
- 可分阶段展示更贴近用户理解的文案：正在准备环境 / 正在启动 WordPress / 正在检查访问状态
...
**说明原则:**
- 不使用“安装完成”，而使用“Websoft9 已完成准备”
- 主动作按钮直达 WordPress 访问入口
- 次动作按钮进入控制台首页
- 明确提示 WordPress 站点初始化仍未结束，将在 WordPress 内继续完成
...
flowchart TD
    ...
    B -->|创建成功| C[Step 3: 启动应用页]
    C -->|点击 开始使用 WordPress| D[加载态: 正在准备...]
    D -->|成功| E[完成态: 交接页]
    E --> F[打开 WordPress 或进入控制台]
...
- 向导已完成后，重访 `/setup` 应直接跳转 `/dashboard`
