...
render_gateway_config() {
  local raw_https_enabled ssl_cert ssl_key https_enabled
  mapfile -t gateway_settings < <(read_platform_gateway_settings)
  raw_https_enabled="${gateway_settings[0]:-false}"
  ssl_cert="${gateway_settings[1]:-/etc/custom/platform-gateway/ssl/websoft9-platform-gateway.cert}"
  ssl_key="${gateway_settings[2]:-/etc/custom/platform-gateway/ssl/websoft9-platform-gateway.key}"
  ...
  render_default_server "$https_enabled"
