...
usage() {
  echo "usage: $0 <start|stop|restart|status> <all|platform-gateway|apphub-api|apphub-media|gitea|portainer|nginx-proxy-manager>" >&2
}
...
case "$service" in
  ...
  nginx-proxy-manager)
    ...
    ;;
  platform-gateway|apphub-api|apphub-media|gitea|portainer)
