...
usage() {
  echo "usage: $0 <start|stop|restart|status> <all|platform-gateway|apphub-api|apphub-media|files-agent|gitea|portainer|nginx-proxy-manager>" >&2
}

npm_start() {
  supervisorctl -c "$supervisor_config" start npm-backend npm-nginx
}

npm_stop() {
  supervisorctl -c "$supervisor_config" stop npm-nginx npm-backend || true
}
...
case "$service" in
  ...
  nginx-proxy-manager)
    ...
    ;;
  platform-gateway|apphub-api|apphub-media|files-agent|gitea|portainer)
